...
namespace Unity.Services.Samples
{
    ...
    public static class UnityServiceAuthenticator
    {
        ...
        public static async Task<bool> TryInitServicesAsync(string profileName = null)
        {
            ...
            {
                return await WaitForInitialized();
            }
            ...
            async Task<bool> WaitForInitialized()
            {
                float deadline = Time.realtimeSinceStartup + k_InitTimeout / 1000f;
        ...
        public static async Task<bool> TrySignInAsync(string profileName = null)
        {
            ...
            async Task<bool> WaitForSignedIn()
            {
                float deadline = Time.realtimeSinceStartup + k_InitTimeout / 1000f;
                while (!AuthenticationService.Instance.IsSignedIn && Time.realtimeSinceStartup < deadline)
                    await Awaitable.NextFrameAsync();
                return AuthenticationService.Instance.IsSignedIn;
            }
        }
